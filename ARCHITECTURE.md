...
- **Alerts** ([ALERTS.md](ALERTS.md)): a scheduled job (Railway cron, every 5 min) queries Langfuse over a 15-min window with a 20-request minimum and posts to a webhook: p95 > 10 s; error rate > 5 %; tool failure rate > 10 %. Each alert has meaning, first checks and mitigation documented and was fired once by fault injection.
