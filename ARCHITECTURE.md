...
- **Correlation id:** generated by the server per request (a client value is kept separately as `client_request_id`, only if it's a UUID). Every log line and span carries it; each answer carries the context's `data_as_of` timestamp; the Anthropic response `request-id` is logged next to it; one agent log line per FHIR call (method, resource type, status, count, ms, UTC time) documents the join to OpenEMR's `api_log` (user + URL + time), since OpenEMR ignores the header.
- **Langfuse:** a trace per question, schedule scan and launch prefetch — a request rejected before the handler (4xx on an unknown session) produces none. Manual spans: prefetch (one per launch, covering all resources), Claude (model, input/output/cache tokens, cost, request id, retries), and each tool. Verification is recorded as a `metric.verification` event carrying the outcome, plus withheld and flag counts on the message span. Langfuse session id = a random `session_ref`, never the handle.
- **Metric definitions:** *error* = HTTP 5xx or a fallback caused by the LLM, schema or verifier; *tool failure* = any FHIR call (prefetch or tool) ending in `error`/`timeout` (403 counted separately); *retry* = any repeated FHIR or Claude call; *queue depth* = waiters on the OpenEMR semaphore, recorded two ways — the instantaneous depth on each FHIR span, and a `queue_wait` count per call that actually waited, which is the form the dashboard can chart and the one that shows saturation building.
...
| `POST /api/sessions` | token in body | `{access_token, patient_id?, kind?}` → `{session_handle, expires_at}` (banner from `GET /api/session`) |
...
Measured on the local stack (32 live eval cases, 2026-09-20): p50 1.4 s, p95 2.6 s, one Claude call per answer, $0.0022 mean cost per answer ([evals](evals/README.md), [AI_COST_ANALYSIS.md](AI_COST_ANALYSIS.md)).
