...
- **Evaluator:** [`agent/copilot/alerts.py`](agent/copilot/alerts.py), deployed as the Railway cron service `alerts` (same source as the agent, schedule `*/5 * * * *`, never restarted). The service's configured start command is `python alerts.py` — [`agent/alerts.py`](agent/alerts.py) is a four-line shim that calls `copilot.alerts.main`, because the start command lives on the Railway service rather than in this repository and so cannot move with the code. By hand the evaluator runs as `python -m copilot.alerts`. It reads Langfuse's public observations API (v2; the legacy traces API is closed to new Langfuse organizations).
...
- **Notification:** a POST to `ALERT_WEBHOOK_URL` (Slack or Discord incoming webhook) when set. One body carries
  both `text` and `content`, the fields Slack and Discord respectively read, so the destination is configuration
  rather than a code change. **Prove it can page before you need it:** `python -m copilot.alerts --test-delivery`
  sends a test message down the same path a firing alert takes and exits 1 if the webhook is unset or unreachable.
  Against the deployed service, without the URL ever leaving Railway:
  `railway run --service alerts -- docker run --rm -e ALERT_WEBHOOK_URL <image> python -m copilot.alerts --test-delivery`. Every run also prints one JSON line (`"alert": true|false`, values, thresholds, window), which Railway log search finds as `alert=true`. No webhook is configured for this project yet (there is no team channel), so Railway logs are where alerts show up.
