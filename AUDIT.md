...
| OPS-4 | medium | The `openemr/openemr:8.5.0` tag was republished during the week: a redeploy pulled a build with version marker 15 (was 14) and ran a database upgrade unannounced. | Deploy logs: `Upgrade database for default from 8.4.0`, `Completed: Upgrade to docker-version 15`. | **Open.** Pin the image by digest; upgrade deliberately. |
| OPS-5 | medium | CCDA import reported 24 successful imports but produced 23 patients: one record silently lost or merged. | Importer output `Finished patients import 24`; `SELECT COUNT(*) FROM patient_data` = 23. | **Open.** Reconcile counts after every import. |
