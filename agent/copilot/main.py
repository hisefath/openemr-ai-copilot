...
def build_info() -> Dict[str, str]:
    ...
    `copilot/_build.py` is generated by the deploy job, never committed — a checked-in version constant goes
    stale the moment someone forgets to bump it, which is worse than no constant at all. Absent, we are running
    from a working tree rather than a CI-built image, and this says so rather than guessing a commit.

    This exists because twice in one day a deploy looked healthy while serving stale code: /health and /ready
    both answer 200 on any build, so neither could distinguish a shipped fix from one that never left the
    laptop. A commit SHA turns that from an inference about timestamps into a string comparison."""
    try:
        from . import _build
        return {"commit": _build.COMMIT, "ref": _build.REF, "built_at": _build.BUILT_AT, "source": "ci"}
    except Exception:
        return {"commit": "unknown", "ref": "unknown", "built_at": "unknown", "source": "working-tree"}
